  ...
  <div class="roles">
    <div class="header">
      ...
      <div class="search" style="display: inline-block">
        ...
        <el-input v-model="search.operation" style="width:200px" placeholder="请输入用户动作"></el-input>
        <el-date-picker v-model="time" type="datetimerange" @change="changeTime" value-format="yyyy-MM-dd HH:mm:ss"
    ...
    <component-table :data="tableData">
      <template v-slot:button="{scope}">
        <!--<el-button v-has="'auth'" type="success" size="mini" style="background: #19be6b" @click="handleAuth(scope)">角色</el-button>
        <el-button v-has="'edit'" type="primary" size="mini" @click="handleEdit(scope)">编辑</el-button>-->
        <component-popover v-has="'btn-log-delete'" :text="'删除'" :params="scope.row" @ok="handleOk"></component-popover>
      </template>
    </component-table>

    <!--<component-dialog v-if="show" :width="showTable.showTableAuth?30:50" :title="optionText" :visible.sync="show" :footer="false">
  ...
  data() {
    return {
      ...
      tableData: {
        ...
        // 分页
        page: {
          align: 'right', total: 1, size: 10, currentPage: 1, currentChange: (currentPage) => {
            this.$nextTick(() => {
        ...
        selectionChange: (params) => {
          this.ids = params.map(item => item.operId);
  ...
  methods: {
    ...
    labelInit() {
      this.tableData.tableLabel = [
        ...
        {prop: 'operId', title: '序号', fixed: 'left', width: 60, align: 'center'},
    ...
    handleOk(params) {
      this.ids = [params.operId]
      console.log("ids",this.ids)
      this.requestDelete(this.ids);
    },
    handleDelete(ids) {
