    ...
    initWeekDLFun() {
      ...
      this.candWeek.setOption({
        tooltip: {
          ...
          formatter: function(datas) {
            let data = datas[0];
            let avgPrice = data.data[1];
            return [
              `${data.name}<br/>`,
              `一周最低价: <span style='color:${
                  data.data[1] > avgPrice ? "#FD1050" : "#0CF49B"
              }'>${data.data[1]}</span><br/>`,
              `周一开盘价: <span style='color:${
                  data.data[2] > avgPrice ? "#FD1050" : "#0CF49B"
              }'>${data.data[2]}</span><br/>`,
              `一周最高价: <span style='color:${
                  data.data[3] > avgPrice ? "#FD1050" : "#0CF49B"
              }'>${data.data[3]}</span><br/>`,
              `周五收盘价: <span style='color:${
                  data.data[4] > avgPrice ? "#FD1050" : "#0CF49B"
              }'>${data.data[4]}</span><br/>`
            ].join("");
          }
        },
