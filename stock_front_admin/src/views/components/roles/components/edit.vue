...
export default {
    ...
    created (){
        this.$nextTick(() => this.request(getPermissionsTree,{},this,data => {
            ...
            // console.log(this.form.formFields.id,'角色id',JSON.stringify(this.form.formFields));
            //设置默认选中的节点
            //根据当前角色id获取已经选中的权限id集合
            // this.defaultCheckedKeys=['1236916745927790564'];
        },false));
    },
  methods:{
      // 角色编辑后， 提交数据
      onsubmit(form,item){
        this.handleSubmit(edit,form,this,data => {
          this.$emit('handleGetTableData');

          // data 编辑后后台接口返回数据  ，包含最新的导航栏信息

          //TODO 调用该方法 更新导航信息
          // console.log('datadata:', data)

          this.setNewMenus(data)
        });
      },
      //  角色管理编辑后  更新左侧导航栏数据
      setNewMenus(newMenus){
        ...
        this.$store.commit("siderList", newMenus);
