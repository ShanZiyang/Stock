...
import SidebarLogo from './slider/Logo.vue'
export default {
    name:'Layout',
    components:{
        SliderBar,
    ...
    /*beforeCreate () {
        let first = this.$store.getters.routes;
        let url  = '/401';
    ...
    created() {
      // debugger;
        ...
        console.log(this.siderBarList,'侧边栏');
        if (this.$store.getters.routes && this.$store.getters.routes.length > 0){
            let homeTabBtn = {name:'首页',path:'/',tabBtnActive:true};
