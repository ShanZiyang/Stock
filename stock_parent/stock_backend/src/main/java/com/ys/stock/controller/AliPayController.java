...
public class AliPayController {
    ...
    @Log(title = "支付宝下单",businessType = BusinessType.INSERT)
    @PostMapping(value = "/trade/page/pay/{productId}")
...
//        我们将form表单字符串返回给前端程序，之后前端将会调用自动提交脚本，进行表单的提交
     ...
     */
    @ApiOperation("用户取消订单")
    @Log(title = "取消订单",businessType = BusinessType.CANCELORDER)
     ...
     */
    @ApiOperation("申请退款")
    @Log(title = "申请退款",businessType = BusinessType.REFUND)
    ...
    public R queryTradeBill(
        ...
        return R.ok("获取账单url成功", downloadUrl);
    }
